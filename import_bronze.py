...
MONTH_LAST_DAY = {"03": "31", "06": "30", "09": "30", "12": "31"}

# Map source filename -> bronze table name
SOURCE_FILES = {
...
def download_and_extract(year: int, quarter: int) -> tuple[str, str]:
    """Returns (temp_dir, data_dir, source_url)."""
    url = quarter_url(year, quarter)
...
def ingest_file(
    ...
    print(f"    {fname} -> {table_name}: {len(rows)} rows{f' ({errors} batch errors)' if errors else ''}")
    return len(rows)
